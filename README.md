...
`stock_rankings.json`（個股雙強排行）前端已整合完成：台股類股／上櫃類股面板的
族群清單與 canvas 上的產業點可下鑽看該產業的雙強個股排行（見下方〈個股雙強
排行〉小節末尾的操作說明），複合類指數與全球面板無個股資料、不提供下鑽入口。
...
> **這是觀察工具，不是買賣建議。**
>
> ⚠ **「剛轉強」訊號已回測兩次、兩次皆陣亡**（2026-07-18／19，見概念測試單
> CT-20260718-01、CT-20260718-02）：機械跟單**會賺錢，但無證據跑贏買進持有加權
> 指數**——(1) 雙強前 3 檔持有 40 日：60 訊號／169 筆、PF 2.89、平均 +9.57%，
> 但**平均超額 −5.54%**；(2) 加 200MA 多頭濾網跨十年重測：761 訊號／2,085 筆、
> PF 1.52、淨 +2.91%（t=5.78 顯著），但**超額 −0.01%**（t≈−0.02，統計上與零
> 無別）。**賺到的是 beta 不是 alpha。**
>
> 註：先前此處寫「未經任何回測驗證」是不準確的——**測過且失敗，是負面證據，
> 不是無證據**（2026-07-25 更正）。未驗的是空頭／盤整 regime 的防禦價值。
>
> 排行純粹是「相對強度座標」的機械式排序，不代表個股基本面、未來報酬或任何
> 形式的操作建議；使用前請自行判斷並承擔風險。
...
`web/rrg.html`：台股類股／上櫃類股面板下，族群清單有個股資料的產業名稱旁會
顯示「▸」，點名稱（或點 canvas 上的產業點）即可下鑽該產業的雙強個股 TOP 10；
複合類指數與全球面板無個股資料、不顯示下鑽入口。`streamlit_app.py`：iframe
下方「⚡ 雙強個股排行」摺疊區可直接用市場／產業／週期三個篩選器查表。
...
`data/tpex_indices.csv`、（若存在）`data/global_prices.csv` 與（若存在）
`data/stock_rankings.json`，重用 `compute_rrg.py`／`compute_tpex.py`／
`compute_global.py` 的計算核心，提供互動式 RRG 動畫（基準／週期／回放範圍／
尾巴長度／族群篩選／面板切換）。`web/rrg.html` 為另一位開發者維護的本機版
頁面；雲端版把 `window.RRG_DATA`、`window.RRG_DATASET_TPEX`、
`window.RRG_DATASETS_GLOBAL` 與 `window.RRG_STOCK_RANKINGS` 動態算好後內嵌進
同一份 `rrg.html` 原始碼再用 `st.iframe` 整頁嵌入，兩邊共用同一份前端邏輯；
`data/stock_rankings.json` 本身是純讀取（不重算），iframe 之外另有一個
「⚡ 雙強個股排行」摺疊區直接用 `st.dataframe` 呈現同一份資料。
